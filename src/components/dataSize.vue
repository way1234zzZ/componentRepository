<template>
  <div id="chart1" class="chart">
    <h2>数据量</h2>
    <div class="topBox">
      <div id="collectionCapacity">
        <span id="collectionCapacityTitle">
          数据容量（TB）：
        </span>
        <span id="collectionCapacityNum">17529</span>
      </div>
      <div id="collectionNum">
        <span id="collectionNumTitle">
          数据数量（条）：
        </span>
        <span id="collectionNumNum">105635</span>
      </div>
    </div>
...
export default {
        ...
        series: [{
          ...
          itemStyle: {
            normal: {
              borderWidth: 8,
              shadowBlur: 30,
              shadowColor: "rgba(251, 249, 249, 1)",
              label: {
                show: true,
                position: 'outside',
                color: '#ddd',
  ...
  methods: {
    initChart() {
      this.myChart = echarts.init(document.getElementById("DataClassification"));
      ...
      this.dataSizeOption.graphic.elements[0].style.image = img;
      this.myChart.setOption(this.dataSizeOption);
...
#collectionCapacityTitle {
  color: white;
  font-size: 0.2rem;
  letter-spacing: 0.05rem;
  padding-left: 10%;
}
#collectionCapacityNum {
  color: white;
  font-size: 0.2rem;
  letter-spacing: 0.05rem;
}
